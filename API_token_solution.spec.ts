...
test.describe('API-based authentication with token extraction', () => {
  ...
  test.beforeAll(async () => {
    ...
    const getTokenResponse = await apiContext.get('https://auth.ted.com/session/new.json?context=ted.auth%2Fsign-in');
    
    const getTokenData = await getTokenResponse.json();
    const authenticity_token = getTokenData.authenticity_token;

    if (!authenticity_token) {
      throw new Error('Failed to retrieve authenticity_token');
    }

    const loginResponse = await apiContext.post('https://auth.ted.com/session.json?context=ted.auth%2Fsign-in', {
      form: {
        'user[email]': 'criobubble@gmail.com',
